...
function prepareSignature() {
	var kp = KeyPair.create(fixPrivateKey(SENDER_PRIVATE_KEY3));
...
function mosaicTransferRequest(){
	...
	var mosaicFee = 0;
	for (var m of MOSAICS) {
		var numNem = calcXemEquivalent(MULTIPLIER * 1000000, m.quantity, m.supply, m.divisibility);
		var fee = CALC_MIN_FEE(numNem);
		mosaicFee += fee;
	
	}
	mosaicFee = (mosaicFee * 5) / 4;
	var totalFee = (msgFee + mosaicFee) * 1000000;
